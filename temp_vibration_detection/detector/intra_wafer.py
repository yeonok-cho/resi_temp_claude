"""
Intra-wafer temperature vibration drift detection using a CUSUM chart.

Detects when temperature roughness increases progressively within a single
wafer's bonding sequence — a signal that vibration is worsening chip by chip.
...
def detect_intra_wafer_drift(
    ...
    """
    Run CUSUM on the intra-wafer temperature roughness sequence to detect
    progressive vibration increase within a single wafer.
    ...
    ----------
    wafer_features :
        Temperature features extracted for all chips in the wafer.
    group_key :
    ...
    roughness = wafer_features.roughness_sequence()
    n = len(roughness)
