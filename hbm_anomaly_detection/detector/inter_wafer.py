"""
Inter-wafer drift detection using EWMA control charts.

Monitors wafer-level roughness summary statistics across consecutive wafers
to detect slow, multi-wafer upward trends that would be invisible within
any single wafer.
...
class InterWaferState:
    """
    Stateful EWMA tracker for one (group_key, channel) combination.
...
class InterWaferTracker:
    """
    Tracks per-group EWMA state for inter-wafer drift detection.

    One tracker instance covers all groups; state is keyed by
    (group_key, channel).
    """
    config: InterWaferConfig = field(default_factory=InterWaferConfig)
    _states: dict[tuple, InterWaferState] = field(default_factory=dict, init=False)

    def _key(self, group_key: GroupKey, channel: str) -> tuple:
        return (*group_key, channel)

    def _get_state(self, group_key: GroupKey, channel: str) -> InterWaferState:
        k = self._key(group_key, channel)
        if k not in self._states:
            self._states[k] = InterWaferState()
    ...
    def update_and_detect(
        ...
        """
        Feed one wafer's mean roughness into the EWMA chart and check for alarm.
        ...
        """
        state = self._get_state(group_key, channel)
        cfg = self.config
